...
def test_clean_rules_save(browser):
    ...
    print(len(rows))
    for row in rows:
        if row.find_elements(By.TAG_NAME,"td")[0].text == name1:
            result = False
            break
        else:
            result = True
    ...
    if commit.is_enabled():
        commit.click()
        time.sleep(3)
    # reason_text = browser.find_element(By.XPATH, "//textarea[@class='DialogTextArea']")
    # if browser.find_element(By.XPATH, "//textarea[@class='DialogTextArea']").is_displayed():
    #     browser.send_keys("for test")
    #     browser.find_element(By.XPATH, "//input[@type='password']").send_keys("Aspen111")
    #     browser.find_element(By.XPATH, "//button[@id='Dialogbox_Bottom_OK_Button_Id']").click()
    time.sleep(5)
    message = BaseFun(browser).get_AlterMessage()
    time.sleep(2)
    assert "Clean rules committed successfully." in message
    # go to equipment\booth check states
    EquipmentPage(browser).go_to_Equipment()
    time.sleep(3)
    browser.find_elements(By.XPATH, "//img[@class='gwt-Image Head_Blue_Style']")[1].click()
